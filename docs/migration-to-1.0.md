# EverOS 1.0 Migration Notes

EverOS 1.0 is a fresh architecture. Historical issues, examples, and
...
old Docker Compose stack are not part of EverOS 1.0.

## Legacy API Mapping

These pre-1.0 routes are no longer supported by the OSS repo:

| Legacy route | EverOS 1.0 replacement |
|---|---|
...
The 1.0 API also changed memory type names. For example,
...
- Treat old OpenClaw and EverMem Cloud plugin examples as archived
  references unless they have been explicitly updated to the 1.0 API.

## Benchmark Guidance

The current LoCoMo reproduction path is documented in
[locomo_benchmark.md](locomo_benchmark.md). The benchmark driver uses
the 1.0 server API: add, flush, search, answer, and evaluate.

Old HyperMem / pre-1.0 evaluation pipeline reports should not be used
as 1.0 bug reports unless they can be reproduced with the current
benchmark commands.
